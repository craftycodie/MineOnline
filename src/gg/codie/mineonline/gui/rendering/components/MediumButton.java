...
public class MediumButton extends GUIObject {
    ...
    public MediumButton(String name, Vector2f position, IOnClickListener clickListener) {
        super(name,
                new TexturedModel(Loader.singleton.loadGUIToVAO(new Vector2f(DisplayManager.scaledWidth(position.x), DisplayManager.scaledHeight(DisplayManager.getDefaultHeight() - position.y)), new Vector2f(DisplayManager.scaledWidth(300), DisplayManager.scaledHeight(40)), TextureHelper.getPlaneTextureCoords(new Vector2f(512, 512), new Vector2f(0, 89), new Vector2f(150, 20))), new ModelTexture(Loader.singleton.loadTexture(PlayerRendererTest.class.getResource("/img/gui.png")))),
    ...
    public void render(Renderer renderer, GUIShader shader) {
        ...
        shader.stop();
        renderer.renderCenteredString(new Vector2f(position.x + 150,  position.y - 32), this.name, mouseWasOver ? new Color(1, 1, 0.627f, 1) : Color.white);
     }
