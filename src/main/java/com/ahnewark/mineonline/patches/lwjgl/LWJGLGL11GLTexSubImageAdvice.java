...
import com.ahnewark.mineonline.patches.mcpatcher.HDTextureFXHelper;
...
public class LWJGLGL11GLTexSubImageAdvice {
    ...
    static void intercept(@Advice.Argument(0) int target, @Advice.Argument(value = 2, readOnly = false) int xOffset, @Advice.Argument(value = 3, readOnly = false) int yOffset, @Advice.Argument(value = 4, readOnly = false) int width, @Advice.Argument(value = 5, readOnly = false) int height, @Advice.Argument(value = 8, readOnly = false) ByteBuffer textureBuffer) {
        ...
        try {
            Object version = ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.client.LegacyGameManager").getDeclaredMethod("getVersion").invoke(null);
            boolean useCustomAnimations = ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.MinecraftVersion").getDeclaredField("useCustomAnimations").getBoolean(version);
