    ...
    static void intercept() {
        try {
            if (LegacyGameManager.thirdPersonView) {
                ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.patches.minecraft.ClassicFOVViewmodelAdvice").getField("callCount").set(null, 1);
                return;
            }
            ...
            if (viewModelFunction != null) {
                ...
                int callCount = (int) ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.patches.minecraft.ClassicFOVViewmodelAdvice").getField("callCount").get(null);
                if (callCount < LegacyGameManager.getVersion().hurtEffectCallsPerFrame - 1) {
                    ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.patches.minecraft.ClassicFOVViewmodelAdvice").getField("callCount").set(null, callCount + 1);
                    return;
                } else {
                    ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.patches.minecraft.ClassicFOVViewmodelAdvice").getField("callCount").set(null, 0);
                }
            }
