...
public class WaterColorizerAdvice {
        ...
        try {
            ...
            if (!texturePack.equals("Default")) {
                try {
                    ...
                    ZipEntry texture = texturesZip.getEntry("misc/watercolor.png");
                    if (texture != null) {
            ...
            if (bufferedImage == null)
                bufferedImage = ImageIO.read(LegacyGameManager.getAppletWrapper().getMinecraftAppletClass().getResourceAsStream("/misc/watercolor.png"));
    ...
    public static void intercept(@Advice.Argument(0) double d, @Advice.Argument(value = 1, readOnly = false) double d2, @Advice.Return(readOnly = false) int returnColor) {
        try {
            ...
            int[] pixels = (int[]) ClassLoader.getSystemClassLoader().loadClass("com.ahnewark.mineonline.patches.minecraft.WaterColorizerAdvice").getDeclaredField("pixels").get(null);
