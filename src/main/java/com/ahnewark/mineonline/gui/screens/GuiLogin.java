...
import com.ahnewark.mineonline.gui.MicrosoftLoginController;
import com.ahnewark.mineonline.gui.sound.ClickSound;
import com.ahnewark.mineonline.utils.LastLogin;
import com.ahnewark.mineonline.gui.components.GuiButton;
import com.ahnewark.mineonline.gui.components.toast.DeviceCodeClipboardToast;
...
public class GuiLogin extends AbstractGuiScreen
{
    ...
    protected void mouseClicked(int x, int y, int button)
    {
        ...
        if (MenuManager.isUpdateAvailable() && y > getHeight() - 20 && y < getHeight() - 10 && x < Font.minecraftFont.width("Update Available!")) {
            ClickSound.play();
            try {
                Desktop.getDesktop().browse(new URI("https://github.com/ahnewark/MineOnline/releases/latest"));
            } catch (Exception ex) {
        ...
        if (y > getHeight() - 10 && y < getHeight() && x > getWidth() - Font.minecraftFont.width("Made by @ahnewark <3")) {
            ClickSound.play();
            try {
                Desktop.getDesktop().browse(new URI("https://linktr.ee/ahnewark"));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
