...
//            renderer.prepare();
    ...
    private static void showLoadingScreen() throws LWJGLException {
        ...
        if (OSUtils.isMac()) {
            // Crude fix for a mac frame issue...
            // Expand and then shrink the frame by 1 pixel, this removes the black bar at the bottom of the frame.
            DisplayManager.getFrame().setSize(DisplayManager.getFrame().getWidth(), DisplayManager.getFrame().getHeight() + 1);
            DisplayManager.getFrame().setSize(DisplayManager.getFrame().getWidth(), DisplayManager.getFrame().getHeight() - 1);
        }
    }
